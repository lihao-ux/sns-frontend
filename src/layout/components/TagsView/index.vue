...
export default {
  ...
  methods: {
    ...
    isAffix(tag) {
      ...
      if (tag.meta && tag.meta.affix) return true;

      // 2. 根据不同角色，动态判定哪个路径需要固定
      const roles = this.$store.getters.roles;
      console.log(roles);

      if (roles.includes("staff") && tag.path === "/staffInfo") return true;
      if (roles.includes("commpany_admin") && tag.path === "/caseInfo/case")
        return true;
      if (roles.includes("admin") && tag.path === "/index") return true;
    },
    isFirstView() {
      try {
        return (
    ...
    initTags() {
      ...
      const roles = this.$store.getters.roles; // 获取角色

      for (const tag of affixTags) {
        if (tag.name) {
          // 【核心逻辑】如果不是 admin，且当前标签是首页，则不进行初始化显示
          if (
            !roles.includes("admin") &&
            (tag.name === "Index" || tag.path === "/index")
          ) {
            continue;
          }
